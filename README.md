...
We instrument the order state machine at its tool boundary. Checkout, fulfillment, receipt delivery, and customer update each become a named stage. Infrai records an exception with a stable`order-agent + stage`fingerprint using one key. In the code,`runOrderAgent()`advances state only after a tool succeeds, captures the precise pre-failure state, and rethrows so the caller still owns recovery. That avoids silent duplicate deliveries on retry.

## Run the complete path

Use Node 20 or newer. Provide the same`INFRAI_API_KEY`the REST client uses for auth. Missed jobs usually trace back to a missing key.
...
Start the Zod-validated HTTP boundary with`npm run dev`, then send the same order shape to`POST /orders/run`. The endpoint returns final state and completed stages. Malformed bodies get a client response with Zod issues instead of a 500.

## The decision in code

`src/order_workflow.ts`is the reusable wrapper. It accepts domain-shaped tools instead of hiding them behind a generic callback, which keeps the observable action aligned with the business transition. When a tool throws,`infrai.errors.capture`sends`message`, the full`exception`, a grouping fingerprint, and order context to`POST /v1/errors/capture`. That is the postmortem breadcrumb.

The one real gotcha is retry identity. An agent may repeat the same stage, so each capture derives`idempotency_key`from order id, stage, and attempt, and the client also sends that value as the idempotency header. A 429 response observes`Retry-After`when present and otherwise uses exponential backoff. Because the response envelope is decoded before status handling, ordinary API rejections retain their structured message and HTTP status. This is how we stop duplicate delivery pages.
...
The focused test supplies order`ord_42`and makes receipt delivery throw after checkout and fulfillment. The expected decision is one capture fingerprinted as`order-agent + receipt`, context showing`state_before_failure: fulfilled`, and no call to the customer-update tool. That asserts no duplicate customer notification.
